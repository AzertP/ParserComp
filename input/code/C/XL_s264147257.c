...
int main(void){
  ...
  scanf("%lld%lld",&n,&m);
  for(i=0;i<m;i++){
    scanf("%lld%lld",&g[3*i].st,&g[3*i].fi);
    g[3*i].kr=1;
    g[3*i+1].st=g[3*i].st+n;
    g[3*i+1].fi=g[3*i].fi+2*n;
    g[3*i+1].kr=0;
    g[3*i+2].st=g[3*i].st+2*n;
    g[3*i+2].fi=g[3*i].fi;
    g[3*i+2].kr=0;
    g[3*i].fi+=n;
  }
  qsort(g,3*m,sizeof(g[0]),sortfnc);
  makemkj(g,x,3*m);
  //for(i=0;i<3*m;i++){
  //  printf("%lld %lld %lld\n",g[i].st,g[i].fi,g[i].kr);
  //}
  scanf("%lld%lld",&a,&b);
  w=dijkstra(a,b,3*n,g,x);
  if(w==llinf){printf("-1\n");return 0;}
  printf("%lld\n",w);
  return 0;
}
