...
ll solve_decrease(const vi &p)
{
    const int n = SZ(p);
    int last_found_number = n + 1;
    
    for (int i = n - 1; i >= 0; --i)
    {
        if (last_found_number == p[i] + 1)
        {
            --last_found_number;
        }
    }
    
    return last_found_number - 1;
}

ll solve_increase(vi p)
{
    const int n = SZ(p);
    ...
    return solve_decrease(p);
}

ll solve_old(const vi &p)
{
    return min(solve_decrease(p), solve_increase(p));
}

vi get_index_to_sequential_forward(const vi &p)
{
    const int n = SZ(p);

    vi index_to_sequential_forward(n, 0);
    
    FI(n)
    {
        index_to_sequential_forward[i] = 0 == i ? 1 : (index_to_sequential_forward[i - 1] * (p[i-1] + 1 == p[i]) + 1);
    }
    
    return index_to_sequential_forward;
}

vi get_index_to_sequential_backward(vi p)
{
    const int n = SZ(p);
    ...
    auto index_to_sequential_forward = get_index_to_sequential_forward(p);
    
    reverse(ALL(index_to_sequential_forward));
...
ll solve(const vi &p)
{
    ...
    vector < int > sequence_length(1 + n, 0);
    
    FI(n)
    {
        sequence_length[p[i]] = sequence_length[p[i] - 1] + 1;
    }
    
    return n - *max_element(ALL(sequence_length));
    
}

bool aux(const vi &p, int depth)
{
    const int n = SZ(p);
...
#ifdef DEBUG
    assert(2 == bf({1, 3, 2, 4}));
    assert(4 == bf({3, 2, 5, 1, 4, 6}));
    assert(5 == bf({6, 3, 1, 2, 7, 4, 8, 5}));
    assert(5 == solve({6, 3, 1, 2, 7, 4, 8, 5}));
    assert(0 == solve({1, 2, 3}));
    assert(2 == solve({1, 3, 2, 4}));
    assert(4 == solve({3, 2, 5, 1, 4, 6}));
