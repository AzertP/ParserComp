...
void calc(S se){
  if(distLP(se,t[n])>r[n]+eps)return;
  
  int S=0,T=0;

  P ph=project( se.first, se.second ,t[n]);
  double key= getTime(se.second-se.first , ph-se.first);

  for(int i=0;i<n;i++){
    
    if(distLP(se ,t[i])>r[i]+eps)continue;

    P pp=project(se.first , se.second , t[i]);
    
    double value = getTime( se.second - se.first , pp - se.first );
    if( value <key)S|=(1<<i);
    else T|=(1<<i);
...
int main(){
  ...
  for(int i=0;i+1<=n;i++){
    for(int j=i+1;j<=n;j++){
      for(int k=-1;k<=1;k+=2){
        vector<S> vec=getTangentCC( C(t[i],r[i]) , C(t[j],r[j]) );
        calc(vec[0]);
        calc(vec[1]);
      }
    }
