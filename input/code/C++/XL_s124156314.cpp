...
bool solve(const bitset<144> maps[13][13], ll h, ll w,
           bitset<144> &state, bool used[13])
{
    bool done = true;
    bitset<144> orig_state;
    ...
    rep (h1, h) {
        ...
        srep (h2, h1+1, h) {
            ...
            used[h1] = used[h2] = true;
            orig_state = state;
            state &= maps[h1][h2];

            if (solve(maps, h, w, state, used)) return true;

            state = orig_state;
            used[h1] = used[h2] = false;
        }
    }
    ...
    rep (v1, w) srep (v2, v1+1, w) {
        if (state.test(v1*12+v2)) {
            if (checked[v1] || checked[v2]) continue;
    ...
    assert(cnt <= w);
    if (cnt < w - 1LL) return false;
    if (!(w & 1LL) && cnt < w) return false;
    if (!(w & 1LL) && cnt == w) return true;

    rep (v, w) {
        if (state.test(v*12+v)) {
...
void _main()
{
    ...
    S bitset<144> maps[13][13];
    bitset<144> zero;

    rep (h1, h) rep (h2, h) {
        maps[h1][h2] = zero;
        rep (w1, w) rep (w2, w) {
            if (grid[h1][w1] == grid[h2][w2] && grid[h1][w2] == grid[h2][w1]) {
                maps[h1][h2].set(w1*12LL+w2);
            }
        }
    }
    ...
    if (h & 1LL) {
        rep (single, h) {
            used[single] = true;
            bitset<144> state = maps[single][single];
            if (solve(maps, h, w, state, used)) {
                cout << "YES" << endl;
                return;
            }
            used[single] = false;
