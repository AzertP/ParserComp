...
void solve() {
    in(N);
    na(A, N);
    int want = ruix(sub(A, 2, N)).back();
    int a = A[0];
    int b = A[1];
    int len = 22;
    auto chk = [&](int a, int b, int x, int want) {
        return (((a - x) ^ (b + x)) == want);
    };
    //linf
    umap_f<P, int> top;
    auto calc = [&](int a, int b, int want) {
        int res = linf;
        rep(x, min(a + 1, bit(len))) {
            if (chk(a, b, x, want)) {
                res = x << len;
                break;
            }
    ...
    calc(a >> len, b >> len, want >> len);
    calc(a >> len, (b >> len) + 1, want >> len);
    calc((a >> len) - 1, (b >> len), want >> len);
    calc((a >> len) - 1, (b >> len) + 1, want >> len);
...
int sister(ll N, vi &A) {
    int want = ruix(sub(A, 2, N)).back();
    int a = A[0];
    int b = A[1];
    rep(x, a) {
        if (((a - x) ^ (b + x)) == want)return x;
    }
    return -1;
}
