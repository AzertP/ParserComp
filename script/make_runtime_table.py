...
---------------------
Produce Table T2: Runtime Summary by Bucket.

Buckets (token_count):
    0–5k, 5k–15k, 15k–30k

Columns:
    Grammar, Bucket, LL(k)_ms, LR(k)_ms, Earley_ms, GLL_ms, RNGLR_ms, BRNGLR_ms

Aggregation:
    Median of median_time_ns (converted to ms) within each (grammar, bucket).
...
*  LL/LR columns show --- when the grammar is not LL(1)/LR(1);
   CONFLICT rows are excluded automatically by the status filter.
...
for grammar, sources in GRAMMAR_SOURCES.items():
    for filename, parser_map in sources:
        ...
        for row in rows:
            ...
            if b is None:
                continue
            raw[grammar][b][col].append(ns_to_ms(row["median_time_ns"]))

# ---------------------------------------------------------------------------
# Aggregate: median within bucket
# ---------------------------------------------------------------------------
...
HEADER = ["Grammar", "Bucket"] + [c + "_ms" for c in OUTPUT_COLUMNS]
out_csv = os.path.join(PROJECT_ROOT, "table_t2_runtime_by_bucket.csv")
