...
class Program {
  static void Main() {
    Dice a = new Dice(Console.ReadLine().Split().Select(int.Parse).ToArray());
    Dice b = new Dice(Console.ReadLine().Split().Select(int.Parse).ToArray());
    Console.WriteLine(a.IsSame(b) ? "Yes" : "No");
  }
}
