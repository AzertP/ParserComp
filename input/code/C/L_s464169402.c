...
int main(void){
    ...
    long long cnt[131072]={0},dp[131072]={0};
    rs g[131072];
    mkj x[131072];
    scanf("%lld%lld",&n,&m);
    for(i=0;i<m;i++){
    ...
    while(pushpt-poppt>0){
      w=pop();
      r=llmax(dp[w],r);
      for(i=x[w].st;i<x[w].st+x[w].kz;i++){
        dp[g[i].fi]=llmax(dp[g[i].fi],dp[w]+1);
        cnt[g[i].fi]--;
        if(cnt[g[i].fi]==0){push(g[i].fi);}
      }
    }
    printf("%lld\n",r);
    return 0;
