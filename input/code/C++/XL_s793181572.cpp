...
int main()
{
	...
	for (ll i = 1; i <= k; i++) {
		ll count = 0;
		if(n - k - 1 >= i - 2 && i - 2 >= 0)
			(count += com.nCr(n - k - 1, i-2)) %= MOD;
		//cout << count << endl;
		if (n - k - 1 >= i - 1 && i - 1 >= 0) {
			(count += (com.nCr(n - k - 1, i - 1) * 2) % MOD) %= MOD;
			//cout << com.nCr(n - k - 1, i - 1) << endl;
		}
		//cout << count << endl;
		if (n - k - 1 >= i && i >= 0)
			(count +=  com.nCr(n - k - 1, i)) %= MOD;
		//cout << count << endl;
		(count *= com.nCr(k - 1, i - 1)) %= MOD;//i分割
		cout << count << endl;
	}
