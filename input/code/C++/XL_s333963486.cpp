...
main() {
    ...
    mint ans = 1;
    if(k == n) {
        cout << accumulate(all(a), (mint)1, [](mint acc, ll t) { return acc * t; }) << endl;
    } else if(b.size() == n && (k & 1)) {
        sort(all(b), greater<ll>());
        cout << accumulate(b.begin(), b.begin() + k, (mint)1, [](mint acc, ll t) { return acc * t; }) << endl;
    } else {
#undef ld
        using ld = _Float128;
        vector<ld> B{0}, C{0};
        for(auto e : b) B.eb(e ? log2f128((ld)-e) : -1e10);
        for(auto e : c) C.eb(e ? log2f128((ld)e) : -1e10);
